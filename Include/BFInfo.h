...
#define BFK_FW_REV			"CFGFWRevision"
#define BFK_DOWNLOADER_REV	"DownloaderRev"
...
#define BFK_DEFAULT_CAM_KCXP	"DefaultKcxpFile"
...
// BitFlow power states (these must be different than Windows
typedef enum _BFPowerStates
{
	BF_POWER_NOT_SET = 0xffff,
} BFPowerStates;
...
// The following section is for user level only definitions.
    ...
    typedef struct _BFInfoUsr
    {
		...
		BFU32				OpenFlags;					// Flags used to open the board.
		...
		BFU32				ControlPacketDataSize;		// Camera's control packet size
...
// The following section is for Kernel only definitions.
	...
	// Function type for individual interrupt resets
	typedef BFVOID	BFC		(*IIResetFuncPtr)				(Bd Board, BFU32 IntType);
	...
	typedef struct _BFDriverInfoKrn
	{
		...
		BFBOOL				SomeOutput;					// Provides some debugging output (less than Verbose)
		...
		// Interrupt Time Queue Global Settings
		BFU32				IntTimeQueueSize;			// Size of intterupt time queue (can be overridden in registry
    ...
    typedef struct _BFInfoKrn
    {
		...
		BFU32				UnhandledUARTInts;			// The number of unhandle UART interrupts (For ISR/DPC debugging)
		...
		BFBoardFuncPtr		BFPower;					// Device handle power Irps
		...
		BFBoardFuncPtr		XXPower;					// Device handle power Irps
		...
		BFBoardFuncPtr		OSPower;					// Device handle power Irps
		BFBoardFuncPtr		OSDeviceControl;			// Handle a user level DeviceIoControl request.

		// Power state flags
		BFU32 SystemPowerState;							// The current system power state
		BFU32 DevicePowerState;							// The current device power state
		BFU32 NewSystemPowerState;						// The new (requested) system power state
		BFU32 NewDevicePowerState;						// The new (requested) device power state
		BFBOOL BoardNeedsRedownload;					// Set to True if the board needs to be re-downloaded after hybernate
		...
		BFBOOL				IntTimeQueueEnabled;		// Set to true to enable int time queueing
		PBFU64				IntTimeQueue[BFIntCount];	// Interrupt time queues
		BFU32				IntTimeQueueInAddr[BFIntCount];	//Interrupt time

		// New Interrupt Scheme
		BFU32				IIList[BFIntCount];				// ISR fills this list with interrupts that have occured
		IIResetFuncPtr		IIResetFuncPtrArray[BFIntCount];	// An array of pointers to reset functions, one for each interrupt type
